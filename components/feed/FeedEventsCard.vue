...
const shortUrlsLengthOfWeb3Ids: boolean = appConfig?.shortUrlsLengthOfWeb3Ids
...
const checkIfSelected = (
  ...
  if (id && typeof(id) === "string") {
    const ifMatch = spasm.checkIfEventHasThisId(
      props.post, standardizeId(id), shortUrlsLengthOfWeb3Ids
    )
    return ifMatch
  } else {
    return false
...
if (id) {
  ...
  isSelectedPost.value = checkIfSelected(p)
}
