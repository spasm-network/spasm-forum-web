<template>
  <div class="p-2">
    ...
    <client-only>
      ...
      <!-- Nerdy details -->
      <div v-if="isArrayWithValues(getPreferredRelaysFromAddress(author))">
        <button @click="toggleNerdyDetails()" class="py-2">
          <div class="text-colorNotImportant-light dark:text-colorNotImportant-dark">
            More nerdy details
            <svg
              :class="{ 'rotate-180': nerdyDetailsShown }"
              class="w-5 h-5 inline" viewBox="0 0 20 20" fill="currentColor"
            >
              <path
                fill-rule="evenodd"
      ...
      <div v-if="nerdyDetailsShown">
        <div
          v-if="getPreferredRelaysFromAddress(author) && getPreferredRelaysFromAddress(author) !== 'none' && isArrayWithValues(getPreferredRelaysFromAddress(author))"
          class=""
        >
          <div class="text-colorNotImportant-light dark:text-colorNotImportant-dark">
            Preferred relays:
          </div>
          <div v-for="relay in getPreferredRelaysFromAddress(author)">
            <div v-if="relay">
              {{ relay }}
            </div>
          </div>
...
const nerdyDetailsShown = ref(false)

const toggleNerdyDetails = () => {
  nerdyDetailsShown.value = !nerdyDetailsShown.value
}
...
if (author && typeof(author) === "string") {
  if (author.length === 64 && isHex(author)) {
    npub = toBeNpub(author)
    nostrHex = toBeHex(author)
  } else if (
    isNostrValue(author)
  ) {
  npub = author
